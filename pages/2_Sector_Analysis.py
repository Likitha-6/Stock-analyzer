...
rows, qualified = [], []
name_lookup_dict = name_df.set_index("Symbol")["Company Name"].to_dict()
...
def icon_hi(v, a):
    if _bad(v) or _bad(a):
        return "❓"
    return "✅" if v >= a else "🟡" if v >= a * 0.9 else "🔴"
...
for sym in sel_syms:
    ...
    row = {
        "Symbol": sym,
        "Company": name_lookup_dict.get(sym, ""),
        "PE": m["PE Ratio"],
        "EPS": m["EPS"],
